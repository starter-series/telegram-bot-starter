...
| 린트 | ESLint 코드 품질 검사 |
| 빌드 | 배포되는 JavaScript 파일의 syntax smoke check |
| 테스트 | coverage threshold가 적용된 Jest |
| Docker 빌드 | 컨테이너 이미지 빌드로 오류 사전 감지 |
| Trivy 스캔 | 컨테이너 이미지의 CRITICAL CVE를 게이트 |
...
2. `tsconfig.json` 추가
3. `npm start`를 compile 후 `dist/`에서 실행하도록 수정
4. `.js` 파일을 `.ts`로 리네임
...
# WEBHOOK_SECRET=  # 웹훅 모드 전용 — WEBHOOK_URL 설정 시 필수
```
...
- CI: `npm audit --audit-level=high`, ESLint, `--coverage` 옵션의 Jest, Docker 빌드, Trivy CRITICAL CVE 스캔, CodeQL(주간), gitleaks(sha256 핀).
- CD: 버전 태그 가드와 자동 GitHub Release 생성을 포함한 Railway / Fly.io 워크플로우.
- 테스트: bootstrap wiring, 환경변수 검증, lifecycle, commands, handlers, health, logging, rate limiting, safe reply를 Jest로 검증합니다 (`npm test` 결과 참고).

**계획됨 (Planned)**
- 없음. 스타터는 출시 시점에 완성된 것이며, 추가 기능은 이 레포가 아니라 다운스트림 프로젝트에서 다뤄야 합니다.

**설계 의도 (Design intent)**
- 런타임 의존성을 둘(`grammy`, `dotenv`)로 제한하고 순수 JavaScript를 채택 — LLM이 이 레포를 확장할 때 ORM·트랜스파일 단계·타입 시스템 때문에 헤매지 않도록.
